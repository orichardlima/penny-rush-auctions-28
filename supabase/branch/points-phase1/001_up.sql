-- ============================================================================
-- PROGRAMA PONTOS SHOW — Fase 1 v3 (entrega em BRANCH, NÃO aplicar em prod)
-- ============================================================================
-- Objetivo: criar a infraestrutura de rastreabilidade e configuração para o
-- Programa Pontos Show, SEM ativar nenhuma flag, SEM alterar saldos, SEM
-- reprocessar histórico, SEM tocar em lotes/bids existentes.
--
-- Auditoria de triggers em public.bids (executada em prod, resultado abaixo):
--   BEFORE INSERT: tr_prevent_invalid_bids, trg_block_bot_when_target_leading
--   AFTER  INSERT: bids_refresh_last_bidders, fury_vault_bid_trigger,
--                  update_auction_on_bid, trg_cancel_scheduled_on_real_bid
--   AFTER  UPDATE: (nenhum)
-- Conclusão: como não existe trigger em UPDATE, o padrão INSERT-primeiro seria
-- seguro, mas mantemos por design a variante "consumir/decidir ANTES do INSERT"
-- e gravar TODOS os snapshots na linha original, para não depender dessa
-- ausência no futuro (item 9 do briefing).
-- ============================================================================
...
-- ─── 2. SEED DE FLAGS (todas seguras, nada ativado) ─────────────────────────
...
-- ─── 4. RPCs ADMIN DE ESCRITA (com auditoria) ───────────────────────────────
...
-- ─── 7. FUNÇÃO consume_bid_lots_for_bid ─────────────────────────────────────
-- Semântica: seleciona lotes FOR UPDATE do usuário, ordenados pela prioridade
-- configurada. Bucket "eligible_paid" = eligible_for_points=true AND
-- purchased_at > points_accrual_started_at (não nulo). Se não conseguir
-- alocar p_amount inteiro, RAISE 'insufficient_lot_balance' e nada é gravado.
-- Retorna TRUE somente se 100% do consumo veio do bucket eligible_paid.
--
-- IMPORTANTE: esta função grava tanto o UPDATE dos lotes quanto o INSERT em
-- bid_lot_consumptions. Deve ser chamada APÓS o INSERT do bid, mas os
-- snapshots de eligibilidade no próprio bid são resolvidos ANTES pelo caller
-- via preview_consume_bid_lots (abaixo). place_bid usa preview → INSERT bid
-- (com snapshots) → commit.
...
DECLARE
  ...
  r RECORD;
BEGIN
  -- Bucket 1: eligible_paid (só se cutoff definido)
...
-- Compatibilidade com o plano: função "tudo-ou-nada" que junta preview+commit.
...
-- Chamadas somente internas (definer) via place_bid.

-- ─── 8. place_bid — versão nova (aditiva) ───────────────────────────────────
-- Ver diff detalhado em place_bid.diff.md.
...
DECLARE
  ...
  v_rows                   json := NULL;
BEGIN
  -- Segurança: só o próprio user OU service_role (para bots/admin via place_bid_as)
  ...
  -- Snapshots calculados ANTES do INSERT (evita UPDATE de metadados)
  ...
  -- Consumo de lotes (opcional). Se falhar, NÃO bloqueia o bid: registra na
  -- fila de reconciliação e segue com débito legado.
  ...
  -- Elegibilidade final: qualquer bot/admin/test/fora-do-piloto zera
  IF v_is_bot OR v_is_admin OR v_is_test OR NOT v_in_pilot OR NOT v_accrual_active_snap THEN
    v_eligible := false;
  END IF;

  -- Débito legado (mantido idêntico ao comportamento atual)
  ...
  -- INSERT canônico do bid, com TODOS os snapshots preenchidos
  INSERT INTO bids(
  ...
  -- Commit dos consumos de lote, agora com bid_id conhecido
  IF v_rows IS NOT NULL THEN
...
-- ─── 9. place_bid_as (service_role apenas) ──────────────────────────────────
...
BEGIN
  ...
  -- p_actor é registrado somente para auditoria futura (não usado ainda)
  PERFORM p_actor;
END $$;
