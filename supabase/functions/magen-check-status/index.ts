...
async function processBidPurchase(supabase: any, purchaseId: string) {
  ...
  // 1. Update bid_purchases status with gateway metadata
  const gatewayConfirmedAt: string | null = purchase.created_at ?? null
  const payloadHash = await crypto.subtle
    .digest('SHA-256', new TextEncoder().encode(JSON.stringify({ purchase_id: purchase.id, tx: txId })))
  ...
  // 2. Canonical credit (creates eligible bid_lot for Pontos Show)
  const { data: creditRes, error: creditErr } = await supabase.rpc(
    'credit_paid_bid_purchase',
    {
      ...
      p_external_payment_id: txId || purchase.id,
      p_gateway_event_id: txId || purchase.id,
