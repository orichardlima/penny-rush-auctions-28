...
-- 2. Recriar como FUNCTION com passagem única (sem loop, sem COMMIT, sem pg_sleep)
...
-- 3. Atualizar o cron job: de CALL para SELECT
SELECT cron.unschedule(48);
SELECT cron.schedule('bot-protection-loop', '* * * * *', 'SELECT public.bot_protection_loop()');
