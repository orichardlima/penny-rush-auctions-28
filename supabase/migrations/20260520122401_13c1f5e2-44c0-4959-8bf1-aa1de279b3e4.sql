-- Índice cobertor: evita heap fetches durante agregações
CREATE INDEX IF NOT EXISTS idx_bids_created_user_cost
  ON public.bids (created_at, user_id) INCLUDE (cost_paid);

-- =========================================================
-- get_financial_summary_filtered (única passada em bids)
-- =========================================================
...
BEGIN
  ...
  -- Única passada em bids: total, bots e receita real (não-bot, cost_paid>0)
  WITH bot_ids AS (
    SELECT user_id FROM public.profiles WHERE is_bot = true
  ),
  bid_window AS (
    SELECT b.user_id, b.cost_paid,
           (b.user_id IN (SELECT user_id FROM bot_ids)) AS is_bot_bid
    FROM public.bids b
    WHERE b.created_at >= v_start_ts AND b.created_at < v_end_ts
  )
  SELECT
    COUNT(*)::int,
    COUNT(*) FILTER (WHERE is_bot_bid)::int,
    COALESCE(SUM(cost_paid) FILTER (WHERE NOT is_bot_bid AND cost_paid > 0), 0)
  INTO v_total_bids, v_bot_bids, v_auction_revenue
  FROM bid_window;

  v_user_bids := GREATEST(v_total_bids - v_bot_bids, 0);

  -- Se não for real_only, usa a coluna agregada de auctions
