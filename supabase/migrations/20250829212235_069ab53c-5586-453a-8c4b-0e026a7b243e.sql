-- CORREÇÃO COMPLETA: Limpeza total e recriação do sistema de leilões
-- PASSO 1: Limpar todos os triggers relacionados a leilões
...
DROP TRIGGER IF EXISTS auction_webhook_unique_trigger ON public.auctions;
...
DROP TRIGGER IF EXISTS log_auction_status_change_trigger ON public.auctions;

-- PASSO 2: Criar as novas funções corrigidas
...
-- Função para sincronização VISUAL apenas
...
-- PASSO 3: Função simplificada de atualização de estatísticas (SEM finalização)
...
-- PASSO 4: Função de proteção simplificada (sem dependência de ends_at)
...
-- PASSO 5: Criar os novos triggers
...
-- PASSO 6: Limpar cron jobs antigos (ignora erros se não existem)
DO $$
BEGIN
  BEGIN
    PERFORM cron.unschedule('finalize-expired-auctions');
  EXCEPTION WHEN OTHERS THEN
    NULL; -- Ignora se não existe
  END;
  
  BEGIN
    PERFORM cron.unschedule('fix-stuck-auctions');
  EXCEPTION WHEN OTHERS THEN
    NULL; -- Ignora se não existe
  END;
  
  BEGIN
    PERFORM cron.unschedule('sync-auction-timers');
  EXCEPTION WHEN OTHERS THEN
    NULL; -- Ignora se não existe
  END;
  
  BEGIN
    PERFORM cron.unschedule('resurrect-incorrectly-finished');
  EXCEPTION WHEN OTHERS THEN
    NULL; -- Ignora se não existe
  END;
END $$;

-- PASSO 7: Configurar novos cron jobs
