  ...
  LOOP
    ...
    IF v_auction.ends_at IS NOT NULL AND v_current_time_br >= v_auction.ends_at THEN
      ...
      ELSE
        ...
        IF v_last_bid_user_id IS NOT NULL AND NOT v_last_bid_is_bot THEN
          -- Último lance é de usuário real — inserir bot para proteger
    ...
    IF v_auction.max_price IS NOT NULL AND v_auction.current_price >= v_auction.max_price THEN
      IF v_auction.company_revenue >= v_auction.revenue_target THEN
        ...
        RAISE LOG '💰 [BOT-LOOP] Leilão "%" finalizado - preço máximo + meta OK', v_auction.title;
      ELSE
        SELECT b.user_id, COALESCE(p.is_bot, false)
        INTO v_last_bid_user_id, v_last_bid_is_bot
        FROM bids b
        ...
        IF v_last_bid_user_id IS NOT NULL AND NOT v_last_bid_is_bot THEN
          ...
          IF v_bot_user_id IS NOT NULL THEN
            ...
            RAISE LOG '🛡️ [BOT-LOOP] Leilão "%" finalizado por max_price - bot protegeu', v_auction.title;
          ELSE
            UPDATE auctions SET status = 'finished', finished_at = v_current_time_br,
              winner_id = NULL, winner_name = NULL
            WHERE id = v_auction.id;
            RAISE LOG '⚠️ [BOT-LOOP] Leilão "%" finalizado sem vencedor (sem bot)', v_auction.title;
          END IF;
        ELSE
          SELECT full_name INTO v_winner_name FROM profiles WHERE user_id = v_last_bid_user_id;
          ...
          RAISE LOG '💰 [BOT-LOOP] Leilão "%" finalizado - preço máximo (bot venceu)', v_auction.title;
        END IF;
      END IF;
      CONTINUE;
    END IF;
