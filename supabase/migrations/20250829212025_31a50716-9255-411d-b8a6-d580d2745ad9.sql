-- CORREÇÃO: Migração defensiva sem tentar remover cron jobs inexistentes
-- FASE 1: Criar nova função unificada que usa APENAS inatividade de lances
...
-- FASE 2: Função de sincronização VISUAL (sem finalização)
...
-- FASE 3: Trigger simplificado para lances (SEM lógica de finalização)
...
-- FASE 4: Trigger de proteção simplificado (sem dependência de ends_at)
...
-- FASE 5: Remover triggers antigos e criar novos
...
-- FASE 6: Configurar novos cron jobs (substituindo os antigos)
-- Finalização por inatividade a cada 10 minutos
...
-- Sincronização visual a cada 2 minutos  
SELECT cron.schedule(
  'sync-visual-timers',
  '*/2 * * * *',
