-- Corrigir bot_protection_loop: usar ends_at como finished_at na finalização por horário limite
...
BEGIN
  ...
  LOOP
    ...
    IF v_auction.ends_at IS NOT NULL AND v_current_time_br >= v_auction.ends_at THEN
      ...
      -- Usar ends_at como finished_at para horário natural
      ...
      RAISE LOG '⏰ [BOT-LOOP] Leilão "%" finalizado - horário limite (ends_at: %)', v_auction.title, v_auction.ends_at;
      CONTINUE;
    END IF;
