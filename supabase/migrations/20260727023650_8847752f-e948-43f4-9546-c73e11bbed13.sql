-- 1. Ajustar points_rules existente
ALTER TABLE public.points_rules
  ADD COLUMN IF NOT EXISTS rule_code text,
  ADD COLUMN IF NOT EXISTS version integer,
  ADD COLUMN IF NOT EXISTS points_per_block integer,
  ADD COLUMN IF NOT EXISTS created_by uuid;

-- Backfill de linhas legadas (se existirem)
UPDATE public.points_rules
   SET rule_code = COALESCE(rule_code, COALESCE(name, 'LEGACY')),
       version = COALESCE(version, 0),
       points_per_block = COALESCE(points_per_block, 1)
 WHERE rule_code IS NULL OR version IS NULL OR points_per_block IS NULL;

ALTER TABLE public.points_rules
  ALTER COLUMN rule_code SET NOT NULL,
  ALTER COLUMN version SET NOT NULL,
  ALTER COLUMN points_per_block SET NOT NULL;

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname='points_rules_rule_code_version_key') THEN
    ALTER TABLE public.points_rules ADD CONSTRAINT points_rules_rule_code_version_key UNIQUE (rule_code, version);
  END IF;
END $$;
...
-- Semente 12:1 (INATIVA)
INSERT INTO public.points_rules (rule_code, version, name, bids_per_point, points_per_block, multiplier, is_active, metadata)
VALUES ('POINTS_STANDARD', 1, 'POINTS_STANDARD v1', 12, 1, 1, false,
...
BEGIN
  SELECT COALESCE(available_points,0), COALESCE(reserved_points,0), COALESCE(blocked_points,0)
...
-- 11. points_admin_activate_pilot
