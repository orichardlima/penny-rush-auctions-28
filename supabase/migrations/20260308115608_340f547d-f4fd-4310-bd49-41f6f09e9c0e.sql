  ...
  LOOP
    ...
    IF v_auction.ends_at IS NOT NULL AND v_current_time_br >= v_auction.ends_at THEN
      IF v_auction.company_revenue >= v_auction.revenue_target THEN
        -- Meta atingida: qualquer vencedor OK
      ...
      ELSE
        -- Receita NÃO atingida: proteger contra usuário real vencer
        ...
        IF v_last_bid_user_id IS NOT NULL AND NOT v_last_bid_is_bot THEN
          -- Último lance é de usuário real — finalizar com bot como vencedor (SEM inserir bid para evitar conflito com trigger)
          ...
          IF v_bot_user_id IS NOT NULL THEN
            ...
            RAISE LOG '🛡️ [BOT-LOOP] Leilão "%" finalizado por ends_at - bot protegeu (receita insuficiente, sem inserir bid)', v_auction.title;
          ELSE
            UPDATE auctions SET status = 'finished', finished_at = v_auction.ends_at,
          ...
          END IF;
        ELSE
          -- Último lance já é de bot — finalizar com bot
    ...
    -- =============================================
    -- LANCE DE BOT PROBABILÍSTICO (proteção anti-inatividade)
    -- =============================================
    ...
    -- Anti-spam: verificar se bot já deu lance nos últimos 3s
    SELECT EXISTS(
      SELECT 1 FROM bids
    ...
    IF v_bot_user_id IS NOT NULL THEN
      ...
      -- Se preço > valor de mercado, finalizar imediatamente (prejuízo)
