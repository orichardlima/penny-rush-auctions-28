...
CREATE OR REPLACE FUNCTION public.find_orphan_binary_points()
RETURNS TABLE (
  partner_contract_id uuid,
  partner_name text,
  plan_name text,
  leg text,
  orphan_points integer
)
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT pbp.partner_contract_id,
         COALESCE(pr.full_name, 'Sem nome'),
         pc.plan_name,
         'left'::text,
         pbp.left_points
  FROM partner_binary_positions pbp
  JOIN partner_contracts pc ON pc.id = pbp.partner_contract_id
  LEFT JOIN profiles pr ON pr.user_id = pc.user_id
  WHERE pbp.left_child_id IS NULL AND pbp.left_points > 0
  UNION ALL
  SELECT pbp.partner_contract_id,
         COALESCE(pr.full_name, 'Sem nome'),
         pc.plan_name,
  ...
  JOIN partner_contracts pc ON pc.id = pbp.partner_contract_id
  LEFT JOIN profiles pr ON pr.user_id = pc.user_id
  WHERE pbp.right_child_id IS NULL AND pbp.right_points > 0;
$$;

GRANT EXECUTE ON FUNCTION public.find_orphan_binary_points() TO authenticated, service_role;

CREATE OR REPLACE FUNCTION public.reverse_orphan_binary_points(
  p_contract_id uuid,
  p_leg text,
  p_reason_note text
)
...
GRANT EXECUTE ON FUNCTION public.reverse_orphan_binary_points(uuid, text, text) TO authenticated, service_role;
