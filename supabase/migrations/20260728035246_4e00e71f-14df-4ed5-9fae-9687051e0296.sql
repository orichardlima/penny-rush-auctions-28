...
BEGIN
  IF NEW.key IN ('audience_mode','audience_version','pilot_audience') THEN
    INSERT INTO public.points_program_settings_audit (key, table_type, old_value, new_value, actor, created_at)
    VALUES (
      NEW.key,
      'json',
      OLD.value::text,
...
CREATE OR REPLACE FUNCTION public.points_admin_activate_pilot(
  ...
  p_audience_mode text DEFAULT 'pilot'::text
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SELECT value INTO v_webhooks_validated
    FROM public.points_program_settings_bool WHERE key = 'webhooks_validated';
  ...
  VALUES (
    '__activate_pilot__',
    'json',
    to_jsonb(v_rule)::text,
    jsonb_build_object(
      'rule_id', p_rule_id,
      'cutoff', p_cutoff,
      'mode', p_audience_mode,
      'audience_version', v_new_ver,
...
REVOKE ALL ON FUNCTION public.points_admin_activate_pilot(uuid,timestamp with time zone,uuid[],text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.points_admin_activate_pilot(uuid,timestamp with time zone,uuid[],text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.points_admin_activate_pilot(uuid,timestamp with time zone,uuid[],text) TO service_role;