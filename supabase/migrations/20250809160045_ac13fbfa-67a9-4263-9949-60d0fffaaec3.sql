-- Corrigir finalização automática de leilões quando timer zerar (corrigido)
...
BEGIN
    ...
    LOOP
        ...
        RAISE LOG 'Cleanup: Auction % (%) finalized - Winner: % (ID: %)', 
            auction_record.id, auction_record.title, calculated_winner_name, last_bidder_record.user_id;
    END LOOP;
