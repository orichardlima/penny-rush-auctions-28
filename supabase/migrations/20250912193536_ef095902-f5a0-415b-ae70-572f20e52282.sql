-- Diversificar regiões dos bots brasileiros
-- Atualizar 1.202 bots distribuídos geograficamente pelo Brasil

DO $$
DECLARE
    -- Arrays com dados das cidades brasileiras
    cities_data JSONB := '[
        -- SUDESTE (40% - 481 bots)
        -- São Paulo (200 bots)
...
BEGIN
    ...
    LOOP
        ...
        -- Log a cada 100 bots processados
        IF counter % 100 = 0 THEN
            RAISE LOG 'Processados % bots de %', counter, total_bots;
        END IF;
    END LOOP;
    
    RAISE LOG 'Diversificação concluída: % bots distribuídos por % cidades brasileiras', counter, JSONB_ARRAY_LENGTH(cities_data);
    
    -- Estatísticas finais por região
    RAISE LOG 'Distribuição por estados: %', (
        SELECT STRING_AGG(state || ': ' || count, ', ')
        FROM (
            SELECT state, COUNT(*)::TEXT as count
            FROM public.profiles 
            WHERE is_bot = true AND state IS NOT NULL
            GROUP BY state 
            ORDER BY COUNT(*) DESC
        ) stats
    );
