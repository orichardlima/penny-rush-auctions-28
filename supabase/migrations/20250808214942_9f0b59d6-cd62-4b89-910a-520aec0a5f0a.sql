-- Criar uma função para finalizar leilão automaticamente
...
-- Criar trigger para finalizar leilão automaticamente
DROP TRIGGER IF EXISTS trigger_finish_auction_on_timer_end ON auctions;
CREATE TRIGGER trigger_finish_auction_on_timer_end
    AFTER UPDATE ON auctions
    FOR EACH ROW
    EXECUTE FUNCTION finish_auction_when_timer_ends();

-- Adicionar colunas para armazenar informações do ganhador
