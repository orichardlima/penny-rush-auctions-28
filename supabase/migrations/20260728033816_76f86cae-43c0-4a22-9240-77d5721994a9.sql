CREATE OR REPLACE FUNCTION public.points_admin_activate_pilot(p_rule_id uuid, p_cutoff timestamptz, p_pilot_user_ids uuid[], p_audience_mode text DEFAULT 'pilot'::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  UPDATE public.points_rules SET is_active = true, active_from = p_cutoff WHERE id = p_rule_id;
