...
const DailyRevenueBars: React.FC<DailyRevenueBarsProps> = ({
  ...
  return <div className="space-y-2">
      ...
      const displayValue = isTodayPending && day.partnerShare > 0 ? day.partnerShare * dayProgress : day.partnerShare;
      ...
      return <div key={day.date.toISOString()} className="flex items-center gap-2">
            <span className={cn("w-14 text-xs font-medium shrink-0", day.isToday && "text-primary font-bold", isProRataDay && "text-muted-foreground/60")}>
          ...
          !isProRataDay && !day.isToday && (!day.isClosed || day.partnerShare === 0) && "bg-muted-foreground/20", !isProRataDay && isAnimating && day.isClosed && "animate-bar-grow")} style={isProRataDay ? {
            width: '100%'
          } : !day.isClosed ? {
            ...
            animationDelay: `${index * 100}ms`
              ...
              {isProRataDay && <Badge variant="outline" className="absolute right-1 top-1/2 -translate-y-1/2 text-[9px] h-3.5 px-1 bg-amber-50 border-amber-400/50 text-amber-700 font-semibold">
                  <Lock className="h-2.5 w-2.5 mr-0.5" />
                  Pré-contrato
                </Badge>}
              
              {/* Badge Hoje DENTRO da barra */}
              {day.isToday && !isProRataDay && day.isClosed && <Badge variant="outline" className="absolute right-1 top-1/2 -translate-y-1/2 text-[9px] h-3.5 px-1 bg-white/90 border-green-500/50 text-green-700 font-semibold">
                  Hoje
                </Badge>}
            ...
            {/* Valor e Porcentagem */}
            <span className={cn("w-16 md:w-28 text-[10px] md:text-xs text-right shrink-0 tabular-nums", day.isToday && !isProRataDay && "text-primary font-bold", !day.isPast && !day.isToday && "text-muted-foreground", isProRataDay && "text-muted-foreground/50")}>
              {isProRataDay ? <span className="text-amber-600">—</span> : showValue ? <>
                  {day.percentage > 0 && <span className="hidden md:inline text-muted-foreground mr-1">
                      {day.percentage.toLocaleString('pt-BR', {
